...
var Db = require('mongodb').Db,
  ...
  Server = require('mongodb').Server; var _ = require('underscore'); 
...
exports.start = function(delay, numTweets) {
  ...
  // Set Timer for pulling down embedded tweets every minute
...
function pullTweets() {
  ...
  http.get(MY_TIMELINE + '&count=' + NUM_TWEETS, function(res) {
    var data = '';
...
TweetUpdater.on('tweets', function(tweets) {
  ...
  tweetCollection.remove({}, function( err, result) {
    _.each(tweets, function(tweet, id, tweets) {
      http.get(O_EMBED_BASE + tweet.id_str, function(res) {
        var data = '';
        res.on('data', function(chunk) {
          data += chunk;
        });
